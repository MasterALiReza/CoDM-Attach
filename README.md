...
```
codm-bot-modular/
├── 🔴 main.py                    # نقطه ورود اصلی - راه‌اندازی ربات
├── 🔴 requirements.txt          # وابستگی‌های Python
├── 🔴 .env.example              # نمونه تنظیمات
├── 🔴 docker-compose.yml        # راه‌اندازی Docker
├── 🔴 Dockerfile                # ایمیج Docker
├──
├── 📁 app/                      # Factory Pattern و Registry
│   ├── factory.py             # BotApplicationFactory - مدیریت ساخت handlers
│   └── registry/              # سیستم ثبت خودکار handlers
│       ├── user_registry.py
│       ├── admin_registry.py
│       ├── contact_registry.py
│       └── inline_registry.py
├──
├── 📁 config/                   # تنظیمات ربات
│   └── config.py              # تنظیمات اصلی، دسته‌بندی سلاح‌ها، حالت‌های بازی
├──
├── 📁 core/                     # هسته اصلی سیستم
│   ├── database/              # لایه دیتابیس
│   │   ├── database_adapter.py   # Adapter Pattern - پشتیبانی چند دیتابیس
│   │   ├── database_pg.py        # PostgreSQL اصلی
│   │   ├── database_pg_proxy.py  # Proxy Pattern - 174KB متدهای دیتابیس
│   │   └── sql_helpers.py        # ابزارهای SQL
│   ├── cache/                 # سیستم کش
│   │   ├── cache_manager.py      # مدیریت کش عمومی
│   │   ├── smart_cache.py        # Smart Cache با TTL دینامیک
│   │   └── ua_cache_manager.py   # کش مخصوص User Attachments
│   └── security/              # لایه امنیتی
│       ├── rate_limiter.py       # Rate Limiting
│       └── role_manager.py       # RBAC - مدیریت نقش‌ها
├──
├── 📁 handlers/                 # Telegram Event Handlers
│   ├── user/                  # Handlers کاربران
│   │   ├── user_handlers_aggregator.py  # تجمیع‌کننده handlers کاربران
│   │   └── modules/           # ماژول‌های کاربران (جستجو، دسته‌بندی، راهنما، ...)
│   ├── admin/                 # Handlers ادمین‌ها
│   │   ├── admin_handlers_modular.py  # تجمیع‌کننده handlers ادمین
│   │   └── modules/           # ماژول‌های ادمین (آنالیتیکس، اتچمنت، پشتیبانی، ...)
│   ├── contact/               # سیستم تماس و تیکتینگ
│   ├── inline/                # Inline Mode Handlers
│   └── channel/               # مدیریت کانال اجباری
├──
├── 📁 managers/                # Business Logic Layer
│   ├── backup_manager.py      # مدیریت بکاپ خودکار
│   ├── channel_manager.py     # مدیریت کانال‌ها
│   ├── contact_system.py      # سیستم تیکتینگ
│   ├── notification_manager.py  # مدیریت اعلانات
│   └── notification_scheduler.py  # زمان‌بندی اعلانات
├──
├── 📁 utils/                    # ابزارهای کمکی
│   ├── analytics_pg.py        # آنالیتیکس پیشرفته (36KB)
│   ├── attachment_analytics.py  # آنالیتیکس اتچمنت‌ها (33KB)
│   ├── search_fuzzy.py        # Fuzzy Search با Levenshtein
│   ├── suggested_attachments.py  # سیستم توصیه هوشمند
│   ├── data_health_check.py   # بررسی سلامت دیتا (36KB)
│   ├── logger.py              # سیستم لاگینگ پیشرفته
│   ├── error_handler.py       # مدیریت خطاها
│   ├── content_validator.py   # اعتبارسنجی محتوا
│   └── validators.py         # Validators عمومی
├──
├── 📁 scripts/                  # اسکریپت‌های مدیریتی
│   ├── apply_sql.py           # اعمال schema PostgreSQL
│   ├── init_postgres.sql      # Schema اولیه دیتابیس
│   ├── db_check_pg.py         # بررسی سلامت دیتابیس
│   └── run_health_checks.py   # بررسی سلامت کلی سیستم
├──
└── 📁 docs/                    # مستندات
    ├── NEW_FEATURES_IMPLEMENTATION.md  # راهنمای فیچرهای جدید
    └── IMPROVEMENTS_GUIDE.md   # راهنمای بهبودها
```

### 📦 توضیح لایه‌ها

- **app/**: Factory و Registry pattern - مدیریت متمرکز ساخت و ثبت handlers
- **core/**: هسته سیستم - Database، Cache، Security
- **handlers/**: مدیریت رویدادهای Telegram با ساختار ماژولار
- **managers/**: Business Logic - جدا از handlers
...
# بررسی سلامت دیتابیس
python scripts/db_check_pg.py

# بررسی سلامت کلی سیستم
python scripts/run_health_checks.py

# بررسی و تعمیر داده‌های تکراری
python scripts/repair_reports_unique.py

# seed کردن FAQ های پیش‌فرض
